...
require_once("inc/header.php");
    ...
    if ($_POST) {
        if(!empty($_FILES['changePhoto']['name']))
        {
            ...
            $photo = $_POST['pseudo'] . '_' . $_POST['prenom'] . '_' . time() . '-' . rand(1,999) . $_FILES['changePhoto']['name'];
            $photo = str_replace(' ', '-', $photo);
            $photo = str_replace(array('é','è','à','ç','ù'), 'x', $photo);

            // Enregistrons le chemin de notre fichier
            $chemin_photo = RACINE . '/assets/uploads/img/' . $photo;
        ...
        {
            $changePhoto = $_POST['photo_actuelle'];
        }
        else 
        {
            $changePhoto = "default.jpg";
        }
    ...
    if(isset($_GET['a']) && isset($_GET['id']) && $_GET['a'] == "delete" && is_numeric($_GET['id'])) # la fonction is_numeric() me permet de vérifier que le paramètre rentré est bien un chiffre
    {
        ...
        if($result->rowCount() == 1)
        {
            ...
            if($delete_result->execute())
            {
                ...
                if(file_exists($chemin_photo) && $membre['photo'] != "default.jpg") # la fonction fil_exists() me permet de vérifier si le fichier existe bel et bien
                {
    ...
    <div class="starter-template">
        <h1><?= $page ?></h1>
        <div class="card">
            <img class="card-img-top img-thumbnail rounded mx-auto d-block" src="<?= URL?>'assets/uploads/img/'" alt="Card image cap" style="width:25%;">
            <div class="card-body">
